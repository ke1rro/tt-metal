...
void run_kernel(RUNTIME_PARAMETERS params)
{
...

    {
        ...
        ckernel::sfpu::init_scalar_modulo_stationary_combined_research(SCALAR_NORMALIZED_DIVISOR, SCALAR_RECIPROCAL_UP, SCALAR_PHYSICAL_DIVISOR);
        PROFILER_SYNC();
    }
    {
        START_PERF_MEASURE("TILE_LOOP")
        ...
        for (std::uint32_t loop = 0; loop < LOOP_FACTOR; ++loop)
        {
            for (std::uint32_t block_start = 0; block_start < TILE_CNT; block_start += MAX_TILES_DEST)
            {
                ...
                for (std::uint32_t block_tile = 0; block_tile < block_tiles; ++block_tile)
                {
                    ...
                    if constexpr (STATIONARY_PERF_PHASE == 0)
                    {
                        SFPU_UNARY_CALL(
                            DST_SYNC_MODE,
                            is_fp32_dest_acc_en,
                            calculate_scalar_modulo_stationary_normalized,
                            (SCALAR_START_SHIFT, SCALAR_INITIAL_SHIFT, SCALAR_HIGH_MANTISSA, 32),
                            block_tile,
                            ckernel::VectorMode::None);
                    }
                    else if constexpr (STATIONARY_PERF_PHASE == 1)
                    {
                        if constexpr (SFPU_UNARY_OPERATION == SfpuType::fmod)
                        {
                            SFPU_UNARY_CALL(
                                ...
                                (sfpu::ScalarModuloKind::Fmod, SCALAR_DIVISOR_EXPONENT, false, false, 32),
                                block_tile,
                                ckernel::VectorMode::None);
                        }
                        else
                        {
                            SFPU_UNARY_CALL(
                                ...
                                is_fp32_dest_acc_en,
                                calculate_scalar_modulo_stationary_finalizer,
                                (sfpu::ScalarModuloKind::FloorRemainder, SCALAR_DIVISOR_EXPONENT, false, true, 32),
                                block_tile,
                                ckernel::VectorMode::None);
                        }
                    }
