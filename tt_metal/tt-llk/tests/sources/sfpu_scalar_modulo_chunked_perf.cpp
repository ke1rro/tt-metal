...
void run_kernel(RUNTIME_PARAMETERS params)
{
...

    {
        ...
        ckernel::sfpu::init_scalar_modulo_chunked_research(0x40400000u, 0x3eaaaaabu, 0x40400000u);
        PROFILER_SYNC();
    }
    {
        START_PERF_MEASURE("TILE_LOOP")
                ...
                for (std::uint32_t block_tile = 0; block_tile < block_tiles; ++block_tile)
                {
                    ...
                    if constexpr (SFPU_UNARY_OPERATION == SfpuType::fmod)
                    {
                        SFPU_UNARY_CALL(
                            DST_SYNC_MODE,
                            is_fp32_dest_acc_en,
                            calculate_scalar_modulo_chunked_robust_perf,
                            (false, UNSAFE_LANES, 32),
                            block_tile,
