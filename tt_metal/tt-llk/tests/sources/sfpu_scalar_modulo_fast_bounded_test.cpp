...
// Test-only exact Blackhole FastBounded scalar modulo kernel. Input is a
// signed normal FP32 dividend (or signed zero); output is a raw FP32 word.
...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    sfpu::init_scalar_modulo_fast_bounded_research(SCALAR_NORMALIZED_DIVISOR, FAST_WORKING_RECIPROCAL, SCALAR_PHYSICAL_DIVISOR);
    ...
    for (int block = 0; block < params.NUM_BLOCKS; ++block)
    {
        ...
        for (std::uint32_t tile = 0; tile < params.NUM_TILES_IN_BLOCK; ++tile)
        {
            ...
            SFPU_UNARY_CALL(
                ...
                (static_cast<sfpu::ScalarModuloKind>(FAST_BOUNDED_KIND),
                 SCALAR_DIVISOR_EXPONENT,
                 FAST_BOUNDED_DIVIDEND_NEGATIVE,
                 FAST_BOUNDED_DIVISOR_NEGATIVE,
