...
// Test-only end-to-end Blackhole stationary modulo kernel.  Input is a signed
// normal FP32 dividend (or signed zero); output is an opaque raw FP32 word.
...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    sfpu::init_scalar_modulo_stationary_combined_research(SCALAR_NORMALIZED_DIVISOR, SCALAR_RECIPROCAL_UP, SCALAR_PHYSICAL_DIVISOR);
    ...
    for (int block = 0; block < params.NUM_BLOCKS; ++block)
    {
        ...
        for (std::uint32_t tile = 0; tile < params.NUM_TILES_IN_BLOCK; ++tile)
        {
            ...
            SFPU_UNARY_CALL(
                ...
                (static_cast<sfpu::ScalarModuloKind>(COMBINED_KIND),
                 SCALAR_DIVISOR_EXPONENT,
                 COMBINED_DIVIDEND_NEGATIVE,
                 COMBINED_DIVISOR_NEGATIVE,
                 SCALAR_START_SHIFT,
