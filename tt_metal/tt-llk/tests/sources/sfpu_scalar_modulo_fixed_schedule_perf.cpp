...
void run_kernel(RUNTIME_PARAMETERS params)
{
...

    {
        ...
        ckernel::sfpu::init_scalar_modulo_fixed_schedule_research(SCALAR_DIVISOR, SCALAR_INITIAL_INVERSE);
        PROFILER_SYNC();
    }
    {
        START_PERF_MEASURE("TILE_LOOP")
                ...
                for (std::uint32_t block_tile = 0; block_tile < block_tiles; ++block_tile)
                {
                    ...
                    if constexpr (SFPU_UNARY_OPERATION == SfpuType::fmod)
                    {
                        SFPU_UNARY_CALL(
                            DST_SYNC_MODE,
                            is_fp32_dest_acc_en,
                            calculate_scalar_modulo_fixed_schedule_perf,
                            (false, SCALAR_START_SHIFT, SCALAR_HIGH_MANTISSA, ACTIVE_LANES, 32),
                            block_tile,
                    ...
                    else
                    {
                        SFPU_UNARY_CALL(
                            ...
                            is_fp32_dest_acc_en,
                            calculate_scalar_modulo_fixed_schedule_perf,
                            (true, SCALAR_START_SHIFT, SCALAR_HIGH_MANTISSA, ACTIVE_LANES, 32),
                            block_tile,
                            ckernel::VectorMode::None);
                    }
                }
