...
def _run(
    ...
    configuration = TestConfig(
        ...
        templates=[
            ...
            SFPU_UNARY_SCALAR(_bits(scalar)),
        ],
        runtimes=[
...
def test_scalar_modulo_blackhole_diagnostic():
    result, _ = _run([50331656.0] * (16 * 16), MathOperation.Fmod, diagnostic=True)
    ...
    required = {
        ...
        _bits(-4.0): 64,  # before correction and after the no-op positive correction
...
@pytest.mark.parametrize("scalar", [3.0, -3.0])
def test_scalar_modulo_hybrid_adversarial(mathop, scalar):
    values = [
        0.0,
        1.0,
        2.0,
    ...
    divisor = torch.tensor(scalar, dtype=torch.float32)
